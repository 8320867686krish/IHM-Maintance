...
class MakeModelContoller extends Controller
{
    ...
    public function index()
    {
        ...
        $user =  Auth::user();
        if ($role_level != 1) {
            $models = MakeModel::with('hazmat:id,name')->where('hazmat_companies_id',$user->hazmat_companies_id)->get();
         }else{
    ...
    public function store(MakeModelRequest $request)
    {
        try {
            ...
            if (empty($id)){
                $inputData['hazmat_companies_id'] = Auth::user()->hazmat_companies_id;
            }
