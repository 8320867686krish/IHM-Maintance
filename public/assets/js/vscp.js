    ...
    async function convertToImage() {
        ...
        fileReader.onload = async function () {
            ...
            for (let i = 1; i <= pdf.numPages; i++) {
                ...
                const originalViewport = page.getViewport({ scale: 1 }); // scale=1 gives actual size

                const width = originalViewport.width;
                const height = originalViewport.height;

                // Decide scale based on width or height
                let scale;
                console.log(height,width)
                if (width < 500 || height < 1000) {
                    scale = 3; // PDF is small → upscale for better quality
                } else {
                    scale = 1; // PDF is normal or large
                }
